...
def main(VIDEO_PATH,SAVED_DIR):
    ...
    parser.add_argument("--pose_config",
                        default="./mmpose/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/hrnet_w48_coco_256x192.py",
    ...
    #VIDEO_PATH = "https://download.openmmlab.com/mmpose/top_down/hrnet/hrnet_w48_coco_256x192-b9e0b3ab_20200708.pth"
    #SAVED_DIR = "vis_results"
    args.img_root = VIDEO_PATH
    args.out_video_root = SAVED_DIR
    ...
    ##################################################
    df1 = pd.DataFrame({'sec':shoulder_times.keys(),'angle':shoulder_times.values()})
    df2 = pd.DataFrame({'sec':hand_times.keys(),'hand':hand_times.values()})
    df = pd.merge(df1,df2)
    ##################################################
