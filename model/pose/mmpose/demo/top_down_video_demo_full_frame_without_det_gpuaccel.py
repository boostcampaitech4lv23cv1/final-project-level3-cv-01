...
def process_img(frame_resize, img_metas, device):
    ...
    """
    assert frame_resize.shape[1::-1] == tuple(img_metas["ann_info"]["image_size"])
    frame_cuda = torch.from_numpy(frame_resize.copy()).to(device).float()
...
def main():
    ...
    img_metas = prefetch_img_metas(
        pose_model.cfg, (video_origin.width, video_origin.height)
    )
    resize_wh = pose_model.cfg.data_cfg["image_size"]
    video_resize = VideoCapture(
        args.video_path, resize=resize_wh, resize_keepratio=True, pix_fmt="rgb24"
    )
    ...
    with torch.no_grad():
        for frame_resize, frame_origin in zip(
            ...
            pose_results = pose_model(return_loss=False, return_heatmap=False, **data)
