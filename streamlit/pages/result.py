...
confirm_video = './deploy/result'

st.subheader("면접 영상 분석 결과입니다.")

VIDEO_PATH = confirm_video
result = pd.read_csv(f"./{VIDEO_PATH.split('/')[1]}/{VIDEO_PATH.split('/')[2]}/result.csv", index_col=0)
pose_result = pd.read_csv(f"./{VIDEO_PATH.split('/')[1]}/{VIDEO_PATH.split('/')[2]}/pose_result.csv", index_col=0)
eye_result = pd.read_csv(f"./{VIDEO_PATH.split('/')[1]}/{VIDEO_PATH.split('/')[2]}/eye_result.csv", index_col=0)
...
print(st.session_state.face_time)
print(st.session_state.pose_time)
print(st.session_state.eye_time)